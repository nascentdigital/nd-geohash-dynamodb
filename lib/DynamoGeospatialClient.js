...
class DynamoGeospatialClient {
    ...
    constructor({ dynamoDB, cache, hashKey = 'hash', geohashKey = 'geohash', geohashIndex = 'index.geohash',
        locationKey = 'location', hashKeyLength = 8}) {
    ...
    queryAsync(tableName, south, west, north, east) {
        ...
        // get hash ranges
        const hashRanges = this.geohashService.getHashRanges(south, west, north, east, this.cache || null);
        console.log('querying over %d geohash ranges:', hashRanges.length);
        _.forEach(hashRanges, (hashRange) => {

            console.log('   %d -> (%d, %d)', hashRange.hashKey, hashRange.min, hashRange.max);
        });

        // start queries (throttled)
        return Promise
            .map(hashRanges, (hashRange) => queryHashRange(this, tableName, hashRange), { concurrency: 5 })
